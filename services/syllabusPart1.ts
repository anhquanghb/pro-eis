...
/**
 * Service xuất đề cương chi tiết học phần (Phần 1)
 * Kiến trúc: Senior AI Solution Architect
 */
export const syllabusPart1Service = {
  async generateDoc(data: {
    course: any;
    allCourses: any[];
    teachingMethods: any[];
    generalInfo: any;
    knowledgeAreas?: any[]; // Bổ sung mảng chứa thông tin các khối kiến thức
    language: 'vi' | 'en';
  }) {
  ...
  async generateContent(data: {
    ...
    const { course, allCourses, teachingMethods, generalInfo, knowledgeAreas = [], language = 'vi' } = data;
    ...
    const content = [
          ...
          this.createCLOTable(course, generalInfo, language)
        ];
  ...
  // Helper ĐÃ CẬP NHẬT: Tạo Bảng Khối kiến thức tự động tick chéo (Tỷ lệ 42/58, Font 12, Indent 0)
  createBlocksTable(course: any, generalInfo: any, knowledgeAreas: any[], language: string) {
      // Ưu tiên sử dụng blocks/subBlocks trong cấu hình chung
      let blocks: any[] = [];
      ...
      for (let i = 0; i < blocks.length; i += 2) {
          ...
          // Hàm hỗ trợ kiểm tra xem môn học có nằm trong block hay không
          ...
          // Ký tự checkbox: 0x2612 = ☒ (Checked), 0x2610 = ☐ (Unchecked)
          ...
          blockRows.push(new TableRow({
              children: [
                  new TableCell({
                      width: { size: 42, type: WidthType.PERCENTAGE }, // Cột 1: 42%
                      margins: { top: 60, bottom: 60, left: 100, right: 100 },
                      children: [new Paragraph({ children: [new TextRun({ text: `${isChecked1} ${name1}`, size: 24 })] })] // Font 12 (size 24)
                  }),
                  new TableCell({
                      width: { size: 58, type: WidthType.PERCENTAGE }, // Cột 2: 58%
                      margins: { top: 60, bottom: 60, left: 100, right: 100 },
                      children: b2 ? [new Paragraph({ children: [new TextRun({ text: `${isChecked2} ${name2}`, size: 24 })] })] : [] // Font 12 (size 24)
                  })
              ]
          }));
      }
      ...
      return new Table({
          ...
          // KHÔNG indent: Mặc định sẽ căn sát lề trái
          borders: {
              top: { style: BorderStyle.NONE, size: 0, color: "auto" },
              bottom: { style: BorderStyle.NONE, size: 0, color: "auto" },
              left: { style: BorderStyle.NONE, size: 0, color: "auto" },
  ...
  // Tạo Bảng Phân bổ giờ học 3 Cột 
  createHoursTable(hrsTheory: number, hrsSelfStudy: number, hrsExercise: number, hrsDiscussion: number, hrsPractice: number, hrsMidterm: number, language: string) {
      ...
      return new Table({
          width: { size: 70, type: WidthType.PERCENTAGE }, // Chiều rộng bảng 70% trang giấy
          alignment: AlignmentType.LEFT,
          indent: {
              size: 648, // Indent from left 0.45 inches = 648 DXA
              type: WidthType.DXA
          },
          borders: { // Ẩn toàn bộ đường kẻ
              top: { style: BorderStyle.NONE, size: 0, color: "auto" },
              bottom: { style: BorderStyle.NONE, size: 0, color: "auto" },
              left: { style: BorderStyle.NONE, size: 0, color: "auto" },
              right: { style: BorderStyle.NONE, size: 0, color: "auto" },
  ...
  createCLOTable(course: any, generalInfo: any, language: string) {
    const t = language === 'vi' ? {
      code: "Mã",
    ...
    if (clos.length === 0) {
        ...
        clos.forEach((cloText: string, index: number) => {
          ...
          const mappedCodes = (mapping?.objectiveIds || []).map((oId: string) => {
              const mo = generalInfo?.moetInfo?.specificObjectives?.find((x: any) => x.id === oId);
              return mo ? mo.code : '';
          }).filter(Boolean).join(', ');
          ...
          rows.push(new TableRow({
            children: [
              ...
              new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, text: mappedCodes })] }),
            ],
          }));
        });
    }
